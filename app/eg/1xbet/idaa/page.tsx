...
export default function IdaaPage() {
  const reduce = useReducedMotion();
  const methods = [
    { n: 'فودافون كاش', d: 'إيداع فوري من محفظة فودافون كاش', badge: 'الأشهر' },
    { n: 'فوري', d: 'ادفع من أي منفذ فوري في مصر', badge: '' },
    { n: 'انستاباي', d: 'تحويل فوري عبر InstaPay', badge: 'سريع' },
    { n: 'اتصالات كاش', d: 'إيداع من محفظة اتصالات كاش', badge: '' },
    { n: 'أورنج كاش', d: 'إيداع من محفظة أورنج', badge: '' },
    { n: 'العملات الرقمية', d: 'USDT وعملات رقمية تانية', badge: '' },
  ];
  return (
        ...
        <div className="relative mx-auto max-w-5xl px-6">
          ...
          <div className="py-12 md:py-16">
            <motion.h1 initial={reduce ? false : { opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.34, delay: 0.08, ease: EASE }}
              className="font-[var(--font-display)] text-[clamp(2.4rem,6.5vw,4rem)] font-black leading-[1.05] tracking-[-0.04em]">طرق الإيداع <span className="text-[var(--nile)]">في مصر</span></motion.h1>
            <motion.p initial={reduce ? false : { opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: 0.15, ease: EASE }}
              className="mt-5 max-w-xl text-lg leading-[1.6] text-[var(--muted)]">اختار طريقة الدفع اللي تناسبك من الطرق المصرية المدعومة.</motion.p>
          </div>
        </div>
      </section>
      {/* بطاقات طرق دفع — العنصر المميز */}
      <section className="mx-auto max-w-5xl px-6 pb-12">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {methods.map((m, i) => (
            <Reveal key={m.n} delay={i * 0.05}>
              <div className="relative h-full rounded-2xl border border-[var(--line)] bg-[var(--card)] p-6 transition-all hover:border-[var(--gold)] hover:shadow-[0_8px_24px_oklch(26%_0.03_250/0.08)]">
                {m.badge && <span className="absolute left-5 top-5 rounded-full bg-[oklch(74%_0.13_78/0.18)] px-2.5 py-0.5 text-xs font-bold text-[var(--gold-deep)]">{m.badge}</span>}
                <div className="mb-4 grid h-12 w-12 place-items-center rounded-xl bg-[oklch(42%_0.11_245/0.1)] text-xl text-[var(--nile)]">◆</div>
        ...
        <Reveal delay={0.2}><div className="mt-10 text-center"><DownloadCTA big>حمّل وابدأ الإيداع</DownloadCTA></div></Reveal>
      </section>
      <Footer />
    </main>
  );
